...
def infer_fast(net, img, net_input_height_size, stride, upsample_ratio, cpu,
    ...
    return heatmaps, pafs, scale, pad, height, width


def run_demo(net, image_provider, height_size, cpu, track, smooth, save_filename):
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    for img in image_provider:
        hmp, pas, scle, pd, height, width = infer_fast(
            net, img, height_size, stride, upsample_ratio, cpu)
        break
    out = cv2.VideoWriter(save_filename,
                          cv2.VideoWriter_fourcc(*'H264'),
                          image_provider.fps, (width, height))
    for img in image_provider:
        orig_img = img.copy()
        heatmaps, pafs, scale, pad, height, width = infer_fast(
            net, img, height_size, stride, upsample_ratio, cpu)
...
# if __name__ == '__main__':
    ...
    # checkpoint = torch.load('openpose/checkpoint_iter_370000.pth', map_location='cpu')
    ...
    # run_demo(net, frame_provider, 256, True, 1, 1)


def extract_pose(videopath, savename):
    net = PoseEstimationWithMobileNet()
    checkpoint = torch.load(
        'pose/openpose/checkpoint_iter_370000.pth', map_location='cpu')
    load_state(net, checkpoint)

    frame_provider = VideoReader(videopath)

    run_demo(net, frame_provider, 256, True, 1, 1, savename)
